...
const useCalculatePlantKPI = (plant: Asset) => {
  ...
  const plantAttributes = _.flatten(plant.attributes);
  const plantAssignments = _.flatten(plantAttributes.map((p) => p.assignments));
  const productionAssignments = plantAssignments.filter(
...
export default useCalculatePlantKPI;
